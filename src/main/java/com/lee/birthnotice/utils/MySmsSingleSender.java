...
public class MySmsSingleSender extends SmsSingleSender {
  ...
  /**
   * 发送短信重写
   * @param nationCode 国际电话区号
   * @param phoneNumber 电话号码
   * @param templateId 短信模板ID
   * @param params 模板中的参数。{1}、{2}
   * @param sign 签名内容
   * @param extend
   * @param ext
   * @return
  ...
  public SmsSingleSenderResult sendWithParam(String nationCode,
	...
	HTTPRequest req = (new HTTPRequest(HTTPMethod.POST, this.url)).addHeader(
			"Conetent-Type", "application/json").addQueryParameter(
					"sdkappid", this.appid).addQueryParameter(
							"random", random).setConnectionTimeout('\uea60').
			setRequestTimeout('\uea60').
